...
Object xAxisAtCRP; // x axis angle current robot position
             ...
             */
                ...
                if ((recognizedTargetObjects.getTargetObjects().size() < 3)) 
                {
                      ...
                      else // should add a special case. When the robot moves out from the behind of starting position.
                      {
                                ...
                                {
                                    //if(TwoAdjacentPos(Point (0,0), rb))
                                                clear_Flag = 1;
                                                OutOfASR_Flag = 1;
                                }
                      }
                           ...
                           if(clear_Flag ==1)
                           {
                                if(OutOfASR_Flag == 0)
                                {
                                      ...
                                      if(TwopathSegment(RobotPs)) // detect two new path is intersect 
                                      {
                                          ...
                                          if(NextOfOutASR_Flag == 1)
                                          {
                                                NextOfOutASR_Flag = 0;// clear this flag
                                                ...
                                                ListOfPositions.push_back(currentRobotPositionInMFIS); // add current one again
                                                Memories.push_back(currentTransLM); // add current one again
                                    ...
                                    if(OutOfASR_Flag == 1)
                                    {
                                            if(v == w) // if this is the final step, store this one
                                            {
                                                    sprintf(viewFileName, "%s%d%s", "Maps/Offline/LocalMap-", v, ".png");
                                                    plotObjectsOf3Kinds(viewFileName, myrobot.getRobot(), NewLM, positions);
                                                    NewLM = currentView; // new local map, new coordinate system
                                                    ...
                                                    ListOfPositions.pop_back();//delete previous one 
                                                    ListOfPositions.push_back(currentRobotPositionInMFIS); //push current position into the list

                                                    //add this position into all
                                                    //robotPositionsInLMs = addTwoVectorsOfObjects(robotPositionsInLMs, currentRobotPositionInMFIS); // store this position into all positions

                                                    
                                                    Memories.pop_back(); //delete previous one memory
                                                    // combine each local map
                                                    //CombineLocalMaps(NewLM, Point (currentRobotPositionInMFIS[6].X1(), currentRobotPositionInMFIS[6].Y1()), currentRobotPositionInMFIS, xAxisAtCRP.getAngleWithXaxis(), v);
                                                    ...
                                                    for(int i = 0; i < ListOfPositions.size() ; i++)
                                                     ...
                                                     // plot this current local map
                                                     sprintf(LocalMFileName, "%s%d%s", "Maps/Offline/Globalmap-", v, ".png");            
                                                     plotObjectsOf3Kinds(LocalMFileName, robotPositionsInLMs, MapofLMs, robot.getRobot());
                                                    
                                                    
                                                    RobotPs.pop_back(); // delete previous one
                                                    RobotPs.push_back(Point (currentRobotPositionInMFIS[6].X1(), currentRobotPositionInMFIS[6].Y1())); // add current one
                                                    ...
                                                    if(TwopathSegment(RobotPs)) // detect two new path is intersect 
                                                    {
                                                           ...
                                                           for(int i = Memories.size() - 1; i > CrossedP1; i--)
                                                                Memories.pop_back();
                                                           Memories.push_back(currentTransLM); // add last one
                                            ...
                                            else // if this is not final step, delete both this and last one, store next step 
                                            {
                                                    if(v < w)
                                                    {
                                                            NextOfOutASR_Flag = 1;
                                                            ...
                                                            //** store all information about this position **//
                                                            sprintf(viewFileName, "%s%d%s", "Maps/Offline/LocalMap-", v, ".png");
                                                            plotObjectsOf3Kinds(viewFileName, myrobot.getRobot(), NewLM, positions);
                                                            NewLM = currentView; // new local map, new coordinate system
