...
int main(int argc, char** argv)
{
    ...
    ArRobot robot;
    ArSick sick;
    if (!connector.parseArgs() || argc > 1)  //if no connected 
    {
        Aria::logOptions();
        Aria::shutdown();
        Aria::exit(1);
    ...
    char buf[1024]="contineous_input_log ";
    affix(buf);   
//    ArSickLogger myLogger(&robot,&sick,100,1,"contineousInput.log");
    ArSickLogger myLogger(&robot,&sick,100,1,buf);
